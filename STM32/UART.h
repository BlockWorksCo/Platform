...
public:
	...
	void ISR(void)
	{
		...
		if( USART_GetITStatus(usart, USART_IT_TXE) )
		{
			...
			char 	byteToTransmit		= txQueue.Get(dataAvailableFlag);

			if(dataAvailableFlag == true)
			{
				//
				// We have data from the queue, so transmit it via USART1.
				//
				USART_SendData(usart, byteToTransmit);
			}
			else
			{
				//
				// No more data in the queue, stop the tx empty interrupts.
				//
...
private:	
	...
	void initialise()
	{
		...
		USART_ITConfig(usart, USART_IT_TXE, DISABLE); // disable the USART1 transmit interrupt 
